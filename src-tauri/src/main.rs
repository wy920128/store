...
fn init_db() {
    let mut lock: std::sync::MutexGuard<'_, Option<Pool>> = POOL.lock().unwrap();
    if lock.is_some() {
        return;
    }
    let opts: Opts = Opts::from_url(&format!(
        "mysql://{}:{}@{}:{}/{}",
...
fn get_conn() -> Result<PooledConn, String> {
    let lock: std::sync::MutexGuard<'_, Option<Pool>> = POOL.lock().unwrap();
    let pool: &Pool = lock.as_ref().ok_or("数据库未连接")?;
    pool.get_conn().map_err(|e: Error| e.to_string())
}
...
fn get_software_by_category(
    ...
    let mut sql: String = String::from(
        "SELECT s.id,s.name,s.package,s.version,s.size,s.icon_url,s.download_count,s.provider_department,s.provider,s.top,s.status,s.created_time,s.updated_time,s.deleted_time
         FROM software s
    ...
    sql.push_str(" GROUP BY s.id ORDER BY s.top DESC, s.download_count DESC LIMIT ?, ?");
...
async fn install_package(software_id: String, package: String) -> Result<String, String> {
    ...
    .map_err(|e: Error| e.to_string())?;
    Ok(format!("✅ 开始安装：{}", package))
}

#[tauri::command]
fn get_system_info() -> Result<SystemInfo, String> {
    let mut conn: PooledConn = get_conn()?;
    let info: Option<SystemInfo> = conn
        .query_map(
            "SELECT 
                name,
                major,
    ...
    Ok(info.unwrap_or(SystemInfo {
        name: "UOS内网应用商店".to_string(),
        major: 1,
        minor: 0,
        patch: 0,
        author: Some("王野".to_string()),
        update_log: None,
        created_time: None,
        updated_time: None,
        deleted_time: None,
